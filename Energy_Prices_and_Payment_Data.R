...
winter_fuel_payment <- read_ods("winter_fuel_payments_last_winter.ods", sheet="2_Local_Authority") %>%
  ...
  filter(complete.cases(.))  # Remove NAs early

# Prepare fuel bills from EPC data
...
# Step 1: Join with region
fuel_bills_24 <- fuel_bills %>%
  left_join(local_authority_to_region, by = "local_authority_code") %>%
  filter(complete.cases(.))  # Remove NAs after join
rm(fuel_bills)

# Step 2: Load electricity and gas data (2024/25)
...
fuel_bills_24 <- fuel_bills_24 %>%
  ...
  filter(!is.na(fuel_category))  # Drop unknown fuels

# Step 5: Estimate individual bills
fuel_bills_24 <- fuel_bills_24 %>%
  mutate(
  ...
  filter(!is.na(avg_fuel_bill_annual_individual_estimate))  # Ensure bill calculated

# Step 6: Load local authority average consumption
...
# Step 7: Join average consumption data
fuel_bills_24 <- fuel_bills_24 %>%
...
# Step 8: Estimate based on local average
fuel_bills_24 <- fuel_bills_24 %>%
  mutate(
  ...
  filter(!is.na(avg_fuel_bill_annual_local_average_estimate))
...
fuel_bills_24 <- fuel_bills_24 %>%
  ...
  filter(!is.na(fuel_bill_difference))

# Optional: Plot if values are within a range
